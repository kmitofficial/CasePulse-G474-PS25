...
const MagicBento = ({
    ...
    const fetchDocs = async () => {
      try {
        const res = await fetch(
          "https://scarlett-uninsinuating-nonsyntonically.ngrok-free.app/latest_fused_files"
        );
                  ...
                  <div className="card__content flex flex-col relative text-white">
                    ...
                    {card.type === "toggle" ? (
                      <div className="flex items-center gap-2 mt-2 text-sm">
                        <span
                          className={`text-m font-medium transition-opacity ${
                            !isUS
  ...
  <div className="flex flex-col gap-4 px-2 py-1">
    {[
      ...
      <div key={method.id} className="flex items-center justify-between cursor-pointer">
        <label className="flex items-center gap-3 cursor-pointer text-white [font-size:1.1rem]">
          <input
            type="radio"
            name="model"
            value={method.id}
            checked={selectedModel===method.id}
            onChange={()=>setSelectedModel(method.id)}
                      ...
                      <div className="flex flex-col gap-4 px-2 py-1">
                        {[
                          { id: "bm25", label: "BM25", info: "Lexical Search" },
                          {
                            id: "legalbert",
                            label: "LegalBERT",
                            info: "Semantic Search with legal context",
                          },
                          {
                            id: "bert",
                            label: "BERT",
                          ...
                          >
                            ...
                            <label className="flex items-center gap-3 cursor-pointer text-white [font-size:1.15rem]">
                              <input
                                ...
                                onChange={() =>
                                  setSelectedSearchMethod(method.id)
                                } // use prop callback
                                className="accent-blue-500 cursor-pointer w-4 h-4"
                              />
                              <span>{method.label}</span>
